  ...
  handler: async (ctx, args) => {
    ...
    if (existing.printfulId !== undefined) {
      const managedFieldChanged =
        ...
        // `description` is deliberately NOT here: it is the hand-written copy
        // the storefront leads with, and the sync cannot touch it. The
        // Printful-owned text is `garmentDescription`.
        (normalizedUpdates.garmentDescription !== undefined &&
          normalizedUpdates.garmentDescription !== existing.garmentDescription) ||
      ...
      if (managedFieldChanged) {
        throw new Error(
          "Name, slug, garment description, price, images, variants, and active status are managed by Printful for this product. Edit them in Printful and sync again. The custom description is yours to edit here.",
        );
      }
    }
